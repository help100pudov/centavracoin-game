<template>
    <div id="app">
        ...
        <div id="resources">
            ...
            <div class="resource">{{ shipHealthPercent }}%</div>
        </div>
        <div class="back">
...
export default {
    ...
    data() {
        return {
            resources: {gold: 0, silver: 0, iron: 0, copper: 0},
            meteors: [],
            meteorTypes: ["gold", "silver", "iron", "copper"],
            enemies: [],
            bullets: [],
            enemyBullets: [],
            ship: {
    ...
    methods: {
        ...
        // Автоматический сбор ценных метеоритов
        autoCollectValuableMeteors(radius) {
            const resourcePriority = ['gold', 'silver', 'iron', 'copper'];
                ...
                for (const meteor of valuableMeteors) {
                    ...
                    if (distance < radius) {
                        ...
                        if (this.user && meteor.token) {
                            this.user[meteor.type] += 1;
                        }
                        
                        // Логируем сбор
                        console.log(`[AUTOPILOT] Collected ${meteor.type} meteor`);
                    }
                }
            }
        },
        ...
        // Автоматический сбор метеоритов
        ...
        restartGame() {
            this.cleanupGame();
            this.resources = {gold: 0, silver: 0, iron: 0, copper: 0};
            this.meteors = [];
            this.enemies = [];
            this.bullets = [];
            this.enemyBullets = [];
        ...
        async createMeteor() {
            if (this.canvas) {
                try {
                    const types = ["gold", "silver", "iron", "copper"];
                    const type = types[Math.floor(Math.random() * types.length)];
                    let token = await this.createToken(type);
                    if (token) {
                    ...
                    // Создаем метеорит без токена если API недоступен
                    const types = ["gold", "silver", "iron", "copper"];
                    const type = types[Math.floor(Math.random() * types.length)];
                    const meteor = new Meteor(
                        Math.random() * this.canvas.width,
...
class Meteor {
    constructor(x, y, type, canvasWidth, health, token) {
        this.x = x;
        this.y = y;
        this.type = type;
        this.width = 30;
